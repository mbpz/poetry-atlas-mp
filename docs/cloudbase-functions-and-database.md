...
| Cloud Functions | Node.js 后端服务 | **8 个**云函数（仓库内），封装业务逻辑 |
...
| `analyzePoem` | AI 诗词解析（混元 hy3） | `content`(必填), `title?`, `author?`, `dynasty?`, `annotation?` | `{ ok, text, structured, usage }` | 无 | `pages-sub/info/poem/poem.js` |
...
### 4.2 线上遗留函数（仓库已删，可择机清理）

| 函数 | 状态 | 说明 |
|---|---|---|
| `quiz` | Active（线上） | 答题对战已下线，本地无代码 |
| `community` | Active（线上） | 公开社区已下线，本地无代码 |

小程序已不再调用；可用 `manageFunctions action=deleteFunction` 删除。
